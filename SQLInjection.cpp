...
#include <tuple>
#include <vector>
#include <regex> //Include Regular Expressions
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Using RegEx to detect SQL injections

    // Create RegEx Expression
    std::regex re("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}", std::regex_constants::icase);

    // Detect any SQL Injetion Pattern Match
    if (std::regex_search(sql, re))
    {
        //Display error message
        std::cout << "Warning! SQL Injection Detected!" << std::endl;
        return false;
    }
