...
std::string read_file(const std::string& filename)
{
    ...
    file_text.assign((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
    // Close the file
    ifs.close();
...
{
    // Create struct to hold time data
    std::tm timeInfo;
    time_t rawTime;
    time(&rawTime);
    char buffer[80];
    // Use localtime_s for personal storage buffer of time
    //Use strftime to copy content of pointer into buffer using the correct format; YYYY-MM-DD
    localtime_s(&timeInfo, &rawTime);
    strftime(buffer, 80, "%F", &timeInfo);
    //write name, date, key and data to new file
